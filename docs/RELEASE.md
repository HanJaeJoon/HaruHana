...
- [ ] 광고 배너가 뜨는지 (Expo Go/웹에서는 네이티브 모듈이 없어 확인되지 않는다)
- [ ] 로컬 알림이 실제로 도착하는지. 설정에서 알림 시각을 2~3분 뒤로 바꾸고
      앱을 백그라운드로 보낸 뒤 기다린다. Android 13+ 는 첫 예약 때 권한
      대화상자가 떠야 한다
- [ ] 기기를 재부팅해도 예약이 남아 있는지 (expo-notifications 가
      RECEIVE_BOOT_COMPLETED 로 복구한다)
- [ ] 다크모드에서 무채색 대비가 깨지지 않는지
- [ ] 아이콘/스플래시가 런처와 부팅 화면에 제대로 나오는지 (라이트/다크 두 벌)
- [ ] 자정을 넘겼을 때 "오늘"이 바뀌고 어제 프롬프트가 뜨는지
